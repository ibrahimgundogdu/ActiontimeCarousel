//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace ActionForce.Entity
{
    using System;
    using System.Collections.Generic;
    
    public partial class VDocumentTransfer
    {
        public long ID { get; set; }
        public string DocumentNumber { get; set; }
        public Nullable<int> FromEmployeeID { get; set; }
        public Nullable<int> FromCustomerID { get; set; }
        public Nullable<int> FromBankAccountID { get; set; }
        public Nullable<int> FromCashID { get; set; }
        public Nullable<System.DateTime> FromDate { get; set; }
        public Nullable<int> FromRecordEmployeeID { get; set; }
        public Nullable<int> ToEmployeeID { get; set; }
        public Nullable<int> ToCustomerID { get; set; }
        public Nullable<int> ToBankAccountID { get; set; }
        public Nullable<int> ToCashID { get; set; }
        public Nullable<System.DateTime> ToDate { get; set; }
        public Nullable<int> ToRecordEmployeeID { get; set; }
        public Nullable<int> CarrierEmployeeID { get; set; }
        public Nullable<double> Amount { get; set; }
        public string Currency { get; set; }
        public Nullable<System.DateTime> DocumentDate { get; set; }
        public Nullable<System.DateTime> RecordDate { get; set; }
        public Nullable<int> RecordEmployeeID { get; set; }
        public string RecordIP { get; set; }
        public Nullable<System.DateTime> UpdateDate { get; set; }
        public string UpdateIP { get; set; }
        public Nullable<int> StatusID { get; set; }
        public Nullable<double> ExchangeRate { get; set; }
        public Nullable<double> SystemAmount { get; set; }
        public string SystemCurrency { get; set; }
        public Nullable<int> OurCompanyID { get; set; }
        public Nullable<long> ReferenceID { get; set; }
        public Nullable<bool> IsActive { get; set; }
        public string ActionTypeName { get; set; }
        public Nullable<int> ActionTypeID { get; set; }
        public string Description { get; set; }
        public Nullable<long> ResultID { get; set; }
        public Nullable<int> EnvironmentID { get; set; }
        public Nullable<System.Guid> UID { get; set; }
        public string StatusName { get; set; }
        public string FromEmpFullName { get; set; }
        public string ToEmpFullName { get; set; }
        public string FromCustFullName { get; set; }
        public string ToCustFullName { get; set; }
        public string FromBankAccountName { get; set; }
        public string ToBankAccountName { get; set; }
        public string FromCashName { get; set; }
        public string ToCashName { get; set; }
        public string FromRecEmpFullName { get; set; }
        public string ToRecEmpFullName { get; set; }
        public string CarrierEmpFullName { get; set; }
        public string RecEmployeeName { get; set; }
        public string UpdEmployeeName { get; set; }
        public Nullable<int> UpdateEmployeeID { get; set; }
        public string FromLocationFullName { get; set; }
        public Nullable<int> FromLocationID { get; set; }
        public Nullable<int> ToLocationID { get; set; }
        public string ToLocationFullName { get; set; }
        public string Environment { get; set; }
    }
}
